...
class Home extends Public_Controller
{
    ...
    public function update_view($id = ''){
        ...
        $check_count_views = $this->_data_seemore->check_count_views($id);
        if (empty($check_count_views)) {
            $this->_data_seemore->insert_count_views($id);
        }else{
            $data1['day'] = $check_count_views->day + 1;
            $condit['id'] = $check_count_views->id;
            $this->_data->update($condit,$data1, 'ap_count_views');
        }
