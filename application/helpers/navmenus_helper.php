...
function parentNavBar($classname = '', $id = '', $submenuclass = '')
{
    return menusParent('0', $classname, $id, $submenuclass);
}
...
function menusParent($location, $classname, $id, $submenuclass){
    $ci =& get_instance();
    $ci->load->model('menus_model');
    $ci->load->library('NavsMenu');
    $ci->load->helper('link');
    $menuModel = new Menus_model();
    $q = $menuModel->getMenuParent($location, $ci->session->public_lang_code);
    $menuModel->listmenuParent($q);
    $listMenu = $menuModel->listmenu;
    $navsMenu = new NavsMenu();
    $navsMenu->set_items($listMenu);
    $config["nav_tag_open"]          = "<ul id='$id' class='$classname'>";
