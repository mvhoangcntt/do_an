...
class Detail_model extends APS_Model
{
    ...
    public function get_count_cart(){
        $this->db->select("ap_cart.account_id");
        // $this->db->selectCount("$this->table_cart.account_id");
        $this->db->from("$this->table_cart");
        $this->db->where("$this->table_cart.account_id", $this->session->account['account_id']);
        $query = $this->db->get();//var_dump($this->db->last_query()); exit();
